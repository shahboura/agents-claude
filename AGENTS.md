...
This repository contains customized agents and skills for Claude Code.

## Project Structure

- `.claude/agents/` - Custom Claude subagent configurations
- `.claude/skills/` - Reusable Claude skills
- `.claude/rules/` - Scoped project rules
...
- [ ] Skills are on-demand and scoped to active tasks
- [ ] High-impact skills use manual invocation when appropriate
...
**Agent:** orchestrator
**Summary:** Reviewed internal docs context and prepared repository for first push/release workflow.
- Used doc-driven review from `CHANGELOG.md`, `AGENTS.md`, and `README.md` to validate current narrative and release intent
- Initialized git workflow planning for commit/push to `shahboura/agents-claude` with branch tracking setup
- Captured guidance on when an `upstream` remote is useful vs unnecessary for an influenced (non-fork) project
- Prepared npm publish runbook focused on validation, authentication, versioning, and release verification

### 2026-03-11 - Post-review fixes: delegation completeness, docs, consistency

**Agent:** orchestrator
**Summary:** Fixed 4 issues identified in repo review.
- Expanded orchestrator delegation guidance from 4 → 7 agents (em-advisor, blogger, brutal-critic were missing)
- Added `argument-hint` + `$ARGUMENTS` to `code-review` skill for consistent scoping across all command skills
- Replaced `commands.md` categorized lists with single argument-hint reference table for all 12 commands
- Added `## Skill policy` to `brutal-critic.md` for consistency with all other agents

### 2026-03-11 - Skill enrichment, assertion hygiene, validator hardening

**Agent:** orchestrator
**Summary:** Completed Phases 2–7 of the skills/agents cross-reference plan.
- Enriched all 11 language skills to 6 actionable bullets each; dotnet replaced FluentAssertions with `AwesomeAssertions` + `Shouldly` (API style hints only, no license noise)
- Added `argument-hint` to 9 command skills; deepened 2 thin bodies (architecture-decision → ADR sections; refactor-plan → staged steps + coverage gates + rollback)
- Added Pattern C command routing table to `docs/skills-matrix.md`; made `blogger.md` `Agent` delegation explicit
- Added 3 structural checks to `validate-agents.js`: command skill agent validation, `$ARGUMENTS`/`argument-hint` pairing, trivial body warning
...
- Updated docs to clarify lazy/on-demand skill loading and Claude Code invocation model
...
### 2026-03-04 - Platform-specific agent optimization (8 phases)

- Enhanced platform config with global permissions (`external_directory`, `doom_loop` deny) and instruction glob loading
